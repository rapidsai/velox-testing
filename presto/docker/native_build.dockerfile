...
ARG CUDA_ARCHITECTURES_ALL=OFF
...
RUN --mount=type=bind,source=presto/presto-native-execution,target=/presto_native_staging/presto \
    ...
    if [ "$CUDA_ARCHITECTURES_ALL" = "ON" ]; then \
        echo "Building for all CUDA architectures" && \
        export CUDA_ARCHITECTURES="70;75;80;86;89;90;100;120"; \
    else \
        THIS_ARCH=$(nvidia-smi --query-gpu=cmpute_cap --format=csv,noheader | head -n 1 | sed 's/\.//') && \
        echo "Building for local CUDA architecture ($THIS_ARCH)" && \
        export CUDA_ARCHITECTURES=$THIS_ARCH; \
    fi && \
