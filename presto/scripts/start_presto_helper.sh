...
if is_gpu_variant; then
  ...
  RENDER_ARGS="--template-path $TEMPLATE_PATH --output-path $RENDERED_PATH --num-workers $LOCAL_NUM_WORKERS --single-container $SINGLE_CONTAINER --kvikio-threads $KVIKIO_THREADS --sccache $ENABLE_SCCACHE"
  if [[ "$VARIANT_TYPE" == "gpu-dev" ]]; then
    RENDER_ARGS="$RENDER_ARGS --dev-mode true --config-variant gpu-dev"
  fi
  if [[ -n ${GPU_IDS:-} ]]; then
