...
function duplicate_worker_configs() {
  ...
  # Separate GPU worker containers can use host networking to expose the host
  # ENAs for per-worker SO_BINDTODEVICE. They all advertise the same host IP,
  # distinguished by the unique HTTP ports assigned above, and discover the
  # coordinator through its host-published port.
