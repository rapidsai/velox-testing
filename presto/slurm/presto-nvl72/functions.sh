...
function setup_benchmark {
    ...
    run_coord_image "export PORT=$PORT; export HOSTNAME=$COORD; export PRESTO_DATA_DIR=/var/lib/presto/data/hive/data/user_data; export MINIFORGE_HOME=/workspace/.miniforge3; (command -v apt-get >/dev/null 2>&1 && apt-get update && apt-get install -y jq) || (command -v yum >/dev/null 2>&1 && yum install -y jq) || true; cd /workspace/presto/scripts; ./setup_benchmark_tables.sh -b tpch -d date-scale-${scale_factor} -s tpchsf${scale_factor}; ./analyze_tables.sh -s tpchsf${scale_factor} -H ${HOSTNAME} -p ${PORT}" "cli"
