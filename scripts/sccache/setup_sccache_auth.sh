...
docker build -f "${SCRIPT_DIR}/sccache_auth.dockerfile" -t sccache-auth "${SCRIPT_DIR}"
...
echo "1. Use these credentials with Velox or Presto builds:"
echo "   velox/scripts/build_velox.sh --sccache"
echo "   presto/scripts/start_native_gpu_presto.sh --sccache"
echo
