...
check_nightly_status.py now monitors two consolidated nightly workflows
(velox-nightly.yml and presto-nightly.yml) that each contain multiple
variants (upstream, staging, pinned) and pipeline phases (Deps Build,
Image Build, Test, Benchmark) as jobs within a single run.  Failure labels
follow the format ``{Pipeline} {Phase} / {variant}`` (e.g.
``Velox Deps Build / upstream``, ``Presto Test (Smoke) / staging``).
