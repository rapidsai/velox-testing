...
get_tpch_benchmark_executable_path() {
  ...
  case "$device_type" in
    "cpu")
      echo "/opt/velox-build/${BUILD_DIR_NAME}/velox/benchmarks/tpch/velox_tpch_benchmark"
      ;;
    "gpu")
      echo "/opt/velox-build/${BUILD_DIR_NAME}/velox/experimental/cudf/benchmarks/velox_cudf_tpch_benchmark"
      ;;
  esac
}
