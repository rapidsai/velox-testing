...
Note that CPU and GPU builds require a local dependencies/run-time base Docker image. By default, images are tagged with your username (e.g., `presto/prestissimo-dependency:centos9-$USER`) to avoid conflicts when multiple users work on the same system. The `start` scripts will not create this automatically. It must be obtained manually. Use the `build_centos_deps_image.sh` script to build an image locally, or the `fetch_centos_deps_image.sh` script to fetch a pre-built image from an external source. Note that the latter script currently requires additional credentials not available to third-parties. You can override the default tag using the `--tag` option available in all scripts.
